function demoWindow() {
    ...
    html = `
        <div>
            ...
            <p>极高的可扩展性和开放性，接口丰富并且高效</p>
            <p>每个窗口都有唯一的Handle，易于管理</p>
            <p>根据当前系统状态（非挡位式，平滑地）自适应调整运行压力</p>
            <p>维护一群虚拟窗口，所有窗口的操作都在虚拟窗口对象上进行，只在状态更改时进行同步，以避免频繁直接操作DOM元素导致性能出现严重问题</p>
            ...
            <p>_t()：将窗口的标题实际同步到DOM元素，参数为：（窗口），无返回值（调用我之前需要先修改指定窗口的标题属性，可以使用_a()来查找窗口）</p>
            <p>_A：存储所有窗口的数组</p>
